  ...
  return (
    <div className="w-64 bg-white border-r border-slate-200 flex flex-col shadow-sm z-10">
      ...
      <div className="p-4 border-t border-slate-100 flex flex-col gap-2">
        <button
          onClick={async () => {
            if (confirm("Generate random seed data?")) {
              await api.seedData();
              alert("Data seeded!");
              window.location.reload();
            }
          }}
          className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-slate-50 border border-slate-200 text-slate-600 rounded-xl hover:bg-slate-100 hover:text-slate-800 transition-all font-medium text-sm"
        >
          Seed Data
        </button>
        <button
          onClick={handleExport}
          className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-xl hover:bg-slate-50 hover:border-slate-300 transition-all font-medium shadow-sm active:scale-[0.98]"
        >
